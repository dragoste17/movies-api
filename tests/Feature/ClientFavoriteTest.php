...
class ClientFavoriteTest extends TestCase
{
    ...
    public function testFavoritesList()
    {
        ...
        $response = $this->actingAs($user, 'api')->get('api/' . config('app.api_latest') . '/favorites');
    ...
    public function testFavoritesStore()
    {
        ...
        $user = User::factory()->create();
        $response = $this->actingAs($user, 'api')->post('api/' . config('app.api_latest') . '/favorite/' . $movie->id);
        $response->assertStatus(200);
